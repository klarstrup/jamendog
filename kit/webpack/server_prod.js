...
export default new WebpackConfig()
  .extend({
    "[root]/server.js": conf => {
      // Optimise images
      conf.module.rules
        .find(l => l.test.toString() === /\.(jpe?g|png|gif|svg)$/i.toString())
        .use.push({
  ...
  .merge({
    ...
    entry: {
      javascript: [path.resolve(PATHS.entry, "server_prod.js")],
    },
    ...
    plugins: [
      webpackProgress(
        `${chalk.magenta.bold("ReactQL server bundle")} in ${chalk.bgMagenta.white.bold(
          "production mode",
        )}`,
      ),
